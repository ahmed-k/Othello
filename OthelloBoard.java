...
class OthelloBoard {
private int blacks=0; private int whites=0;
...
isReal = reality;
buildPosition();
for (int i=0; i<64;i++) position[i].modify(original.getTile(i));  
}//copy constructor 
...
if (i>8) { if ((i-9)%8==0) position[i-9].setBottomTiles(null, position[i-1], position[i]);
		   else if ((i-9)%8==7) position[i-9].setBottomTiles(position[i-2], position[i-1], null);
...
int color;
if (isBlackTurn) color=WHITE; else color=BLACK;  
if (position[0].getColor()==color||position[7].getColor()==color||position[56].getColor()==color||position[63].getColor()==color) return -1000;
else return 0;
}//cornerTaken

public boolean cornerCases(int i, int possibleCorner) {
if (possibleCorner!= -1 && position[possibleCorner].getType()==CORNER) return false;
if (position[i].getType()==CORNERGUARD||position[i].getType()==EDGECORNERGUARD) {
if (position[position[i].whichCorner()].getColor()==BLANK) return false; }
...
for (int i=0; i<state.length; i++) state[i]=position[i].getColor(); 
return state;
}//save()
...
for (int i=0; i<64;i++) {position[i].setColor(rebound[i]);
						}//rebound table 
...
int colorDifference; 
{
if (turn==true) colorDifference = blacks/(blacks+whites);
else colorDifference =whites/(whites+blacks);  
}
int evaluation = numberOfMyMoves-5*numberOfYourMoves+colorDifference+cornerTaken();  
return -1*evaluation; 
}//evaluate
...
for (int i=0; i<64;i++) if (position[i].getColor() == POSSIBLE) list.add(i);  
return list; 
}//countPossible() 
...
for (int i=0; i<64; i++) 
if (position[i].getColor() == POSSIBLE) {position[i].clearTargets(); 
...
public void setEdge() {
if (indicator<7) edge = NORTH;
else if (indicator>55) edge = SOUTH;
else if (indicator%8==0) edge = WEST;
else if (indicator%8==7) edge = EAST; 
else edge = 0;
}//setEdge()
...
if (isBlackTurn) color=WHITE; else color=BLACK; 
switch (c){
case 1: if  (position[c+1].getColor()==color && checkEdge(3,4,5,6,7)) return true;
case 6: if  (position[c-1].getColor()==color && checkEdge(0,1,2,3,4)) return true;
case 8: if  (position[16].getColor()==color && checkEdge(24,32,40,48,56)) return true;
case 15: if (position[23].getColor()==color && checkEdge(31,39,47,55,63)) return true; 
case 48: if (position[40].getColor()==color && checkEdge(0,8,16,24,23)) return true;
case 57: if (position[58].getColor()==color && checkEdge(59,60,61,62,63)) return true;
case 55: if (position[47].getColor()==color && checkEdge(7,15,23,31,39)) return true;
case 62: if (position[61].getColor()==color && checkEdge(56,57,58,59,60)) return true;
...
int color;
if (isBlackTurn) color=WHITE; else color=BLACK; 
if (position[a1].getColor() == color||(position[a2].getColor() == color)||(position[a3].getColor() == color)||
	(position[a4].getColor() == color)||(position[a5].getColor() == color)) return false;
if ((position[a1].getColor() == BLANK)&(position[a2].getColor() == BLANK)&(position[a3].getColor() == BLANK)&
	(position[a4].getColor() == BLANK)&(position[a5].getColor() == BLANK)) return false;
...
if ((i==0)||(i==7)||(i==56)||(i==63))
	{
weight=50; type=CORNER; 
	}//if corners 
	
else if ((i==1)||(i==8)||(i==6)||(i==15)||(i==48)||(i==57)||(i==55)||(i==62)) {
type=EDGECORNERGUARD; weight=-150;
}	
else if ((i==9)||(i==14)||(i==49)||(i==54))
{ type=CORNERGUARD;	weight=-99;} 
else if ((i%8==0)||(i%8==7)||(i<8)||(i>55)) {type=EDGE; weight=30;} 
else if ((i%8==1)||(i%8==6)||(i<14)||(i>48)) {type=SQUARE; weight=20;}
else if ((i%8==2)||(i%8==5)||(i<22)||(i>41)) {type=SQUARE; weight=10;}
else weight=1; 
}//setWeight
...
for (int j=0; j<64;j++) {
if (position[i].getType() == CORNER && position[i].getColor()==BLACK && isBlack)  
}
...
for (Integer j : killPossibles) position[j].setColor(BLANK);
}//play()
...
public void invert() {
if (color==BLACK) {setColor(WHITE); blacks--;}
else if (color==WHITE) {setColor(BLACK); whites--;}
}//invert
...
if (isReal) board.update(indicator);
}
