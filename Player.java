...
if (!possible.isEmpty()) {
			...
			while (!click) System.out.print(""); 
			board.humanPass(true);												
			}//if 
			else {board.humanPass(false); System.out.println("Nothing to play by human. Turn skipped.");} 			
					...
					if (result>finalresult) {
											...
											finalindex=present; 
											finalresult=result;}
											else {lastResort.add(present);} 
											} //if new move is better 
					if (present==0||present==7||present==56||present==63) {finalindex=present; finalresult=result;}
					virtual.revert(arrangement); 
					isBlackVirtual = !isBlackVirtual;
					virtual.setup(isBlackVirtual);	
					}//while
					real.setup(isBlack);
					if (finalindex!=-1) {System.out.print("Computer: " + finalindex); real.play(finalindex);} 
					else if (!lastResort.isEmpty()) {int resort=lastResort.remove(0);
													System.out.print("Computer: " + resort);
													real.play(resort);}
					lastResort.clear();
					real.botPass(true);
					}//if possible to play
					else {System.out.println("Nothing to play. Turn is skipped.");
							real.botPass(false);}
			}//else AI turn
...
localresult = minimax(localpresent, depth-1, localfinal, size);
if (localresult*-1 < prevnode) possibleLevel.clear();
if (localresult>localfinal) {localfinal=localresult; 
...
else return virtual.evaluate(isBlackVirtual, index, possibleLevel.size()); 
return -1*localfinal;
}//else 
}//minimax()
